...
class LoRANIN(nn.Module, LoRALayer):
    ...
    def __init__(self, in_dim, num_units, init_scale=0.1, r=0, lora_alpha=1, lora_dropout=0.0, merge_weights=True):
        ...
        LoRALayer.__init__(self, r=r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, merge_weights=merge_weights)
        self.W = nn.Parameter(layer_utils.default_init(scale=init_scale)((in_dim, num_units)), requires_grad=False)
    ...
    def forward(self, x):
        x = x.permute(0, 2, 3, 1)
        y = layer_utils.contract_inner(x, self.W) + self.b
        if not self.merged and self.r > 0:
            y += layer_utils.contract_inner(self.lora_dropout(x), self.merge_AB().T) * self.scaling
...
class LoRAConv(nn.Module, LoRALayer):
    def __init__(self, conv_module, in_channels, out_channels, kernel_size, r=0, lora_alpha=1, lora_dropout=0.0, merge_weights=True, **kwargs):
        ...
        conv_module.__init__(self, in_channels, out_channels, kernel_size, **kwargs)
        # self.conv = conv_module(in_channels, out_channels, kernel_size, **kwargs)
        LoRALayer.__init__(
            self,
            r=r,
        ...
        if r > 0:
            ...
            self.lora_B = nn.Parameter(self.weight.new_zeros((out_channels // self.groups * kernel_size, r * kernel_size)))
            self.scaling = self.lora_alpha / self.r
            # Freezing the pre-trained weight matrix
            self.weight.requires_grad = False
        self.reset_parameters()
    ...
    def reset_parameters(self):
        self.conv_module.reset_parameters(self)
        if hasattr(self, "lora_A"):
            # initialize A the same way as the default for nn.Linear and B to zero
            nn.init.kaiming_uniform_(self.lora_A, a=math.sqrt(5))
            nn.init.zeros_(self.lora_B)
    ...
    def train(self, mode=True):
        self.conv_module.train(self, mode)
        if mode:
            if self.merge_weights and self.merged:
                if self.r > 0:
                    # Make sure that the weights are not merged
...
class LoRAConv2d(LoRAConv, nn.Conv2d):
    def __init__(self, *args, **kwargs):
        # the following two method are all right,  see  Conv2d.mro() for help
        # ConvLoRA.__init__(self, nn.Conv2d, *args, **kwargs)
...
#         print(r, lora_alpha, lora_dropout, merge_weights)
#         LoRALayer.__init__(
#             self,
#             r=r,
#             lora_alpha=lora_alpha,
...
class LoRAConv1d(LoRAConv, nn.Conv2d):
    def __init__(self, *args, **kwargs):
...
class LoRAConv3d(LoRAConv, nn.Conv2d):
    def __init__(self, *args, **kwargs):
