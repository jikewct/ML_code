...
def ddpm_conv1x1(in_planes, out_planes, stride=1, bias=True, init_scale=1.0, padding=0):
    ...
    conv.weight.data = net_utils.default_init(init_scale)(conv.weight.data.shape)
...
class NIN(nn.Module):
    ...
    def __init__(self, in_dim, num_units, init_scale=0.1):
        super().__init__()
        self.W = nn.Parameter(net_utils.default_init(scale=init_scale)((in_dim, num_units)), requires_grad=True)
...
class ResnetBlockDDPM(nn.Module):
    ...
    def __init__(self, act, in_ch, out_ch=None, temb_dim=None, conv_shortcut=False, dropout=0.1):
        ...
        if temb_dim is not None:
            ...
            self.Dense_0.weight.data = net_utils.default_init()(self.Dense_0.weight.data.shape)
